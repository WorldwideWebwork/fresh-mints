<script lang="ts">
  ...
  async function handleLogin(e: SubmitEvent) {
    ...
    try {
      ...
      if (!success) {
        localError =
          authStore.errorMessage || "Invalid credentials. Please verify your login details.";
      }
    } catch (err: any) {
...
>
  ...
  <div class="w-full max-w-md relative z-10 space-y-6">
    ...
    <div class="text-center space-y-2">
      <div
        class="inline-flex items-center justify-center p-3.5 rounded-2xl bg-gradient-to-br from-emerald-500/20 via-cyan-500/20 to-teal-500/20 border border-emerald-500/30 text-emerald-600 dark:text-emerald-400 shadow-lg shadow-emerald-500/10 mb-2"
      >
    ...
    <!-- Login Form Card -->
    <div
      class="p-7 rounded-3xl bg-[var(--fm-surface)]/90 backdrop-blur-2xl border border-[var(--fm-border)] shadow-2xl shadow-black/10 dark:shadow-black/60 space-y-5"
    >
      <!-- Rep Security Badge -->
      <div
        class="flex items-center justify-between p-2.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-xs"
      >
        ...
        <Badge
          variant="success"
          class="text-[10px] font-mono">$300 Bounty</Badge
        >
      </div>
      ...
      >
        ...
        <div class="space-y-1.5">
          <label
            for="fm-username"
            class="block text-xs font-bold text-[var(--fm-text)] uppercase tracking-wider"
          >
        ...
        <div class="flex items-center justify-between pt-1">
          <label
            class="flex items-center gap-2 text-xs text-[var(--fm-text-muted)] cursor-pointer select-none"
          >
            <input
              type="checkbox"
    ...
    <div class="flex items-center justify-center gap-2 text-[11px] text-[var(--fm-text-muted)]">
      ...
      <span>256-Bit Encrypted Session • Auth</span>
    </div>
  </div>
</div>
