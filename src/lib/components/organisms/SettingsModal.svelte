<script lang="ts">
  ...
  import { Settings, Save, Trash2, CheckCircle2, ShieldCheck } from 'lucide-svelte';
...
<Dialog bind:open {onclose} title="Fresh Mints Application Settings" description="Configure registry discovery defaults and Questbook CRM connection" maxWidth="max-w-lg">
  <div class="space-y-5">
    <!-- Questbook CRM Status -->
    <div class="p-4 rounded-xl bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 space-y-2">
      <div class="flex items-center justify-between">
        <span class="text-xs font-bold text-slate-800 dark:text-slate-200">Questbook CRM Status</span>
        <Badge variant="success">Plug & Play Connected</Badge>
      </div>
      <p class="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
        Syncs seamlessly with WordPress Custom Post Type <code class="text-emerald-600 dark:text-emerald-400 font-mono">questbook_contact</code> and standard taxonomy keys.
      </p>
    </div>
