...
<div class="space-y-6">
  ...
  <Card class="p-6 space-y-4">
    <h3 class="text-sm font-bold text-slate-800 dark:text-slate-200 uppercase tracking-wider flex items-center gap-2">
      ...
      <span>Query Parameters & Target Jurisdictions</span>
    </h3>

    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
      <div>
        <label class="text-xs font-semibold text-slate-500 dark:text-slate-400 block mb-1">Target Sector / Profession:</label>
        <Select bind:value={selectedProfession} options={professionOptions} class="text-xs" />
      </div>
