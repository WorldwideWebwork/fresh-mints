...
export interface W4HostingPlan {
  id: W4HostingTier;
  name: string;
  monthlyBaseRate: number; // e.g. 14.99, 34.99, 74.99, 129.99, 299.00, 650.00
  wholesaleMonthlyCost: number; // raw provider wholesale cost e.g. 15.00, 35.00, 60.00
  hardwareSpecs: string; // vCPU, RAM, SSD specs
  twoYearFlatPackagePrice: number; // e.g. 1250, 1650, 2650, 3950, 8500, 18500
  callerCommission: number; // $300.00 to $2500.00 cash bounty per closed deal
  twoYearHostingCost: number; // 24 * wholesaleMonthlyCost
  ...
  domainBuyoutPrice: number; // unencumbered DNS equity transfer
  description: string;
...
export const W4_HOSTING_PLANS: Record<W4HostingTier, W4HostingPlan> = {
  quantum: {
    ...
    hardwareSpecs: '1 vCPU • 512MB RAM • 10GB SSD (No WAF/Staging)',
